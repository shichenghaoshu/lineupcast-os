...
import { getProviderDashboard, type ProviderDashboard, type ProviderItem, type ProviderLog } from "@/lib/api-client";
...
export default function DataPage() {
  ...
  return (
    <div className="min-h-screen">
      <TopBar title="数据源管理" subtitle={`${connected}/${total} 已连接 · API ${dashboard?.apiHealth ?? "checking"}`} />
      ...
      <div className="grid grid-cols-1 gap-4 p-4 sm:p-6 xl:grid-cols-12">
        <div className="space-y-4 xl:col-span-5">
          ...
          <div className="space-y-2">
            {loading && <div className="card text-sm text-[var(--text-muted)]">正在加载数据源...</div>}
        ...
        <div className="space-y-4 xl:col-span-7">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <StatusCard label="API" value={dashboard?.apiHealth ?? "-"} tone={dashboard?.apiHealth === "online" ? "green" : "amber"} />
            <StatusCard label="Sync" value={syncing ? "running" : dashboard?.syncStatus ?? "-"} tone={syncing ? "amber" : "green"} />
            <StatusCard label="Test" value={testing ? "running" : dashboard?.testStatus ?? "-"} tone={testing ? "amber" : "green"} />
          </div>
              ...
              <table className="w-full min-w-[560px] text-xs">
                <thead>
                  <tr className="bg-[var(--bg-primary)]">
                    <th className="px-3 py-2 text-left font-medium text-[var(--text-muted)]">源字段</th>
                    <th className="px-3 py-2 text-left font-medium text-[var(--text-muted)]">目标字段</th>
                ...
                <tbody>
                  {fieldMap.map((field, i) => (
                    <tr key={field.source} className={i % 2 === 0 ? "bg-[var(--bg-card)]" : "bg-[var(--bg-secondary)]"}>
                      <td className="px-3 py-2 font-mono text-[var(--accent-blue)]">{field.source}</td>
                      <td className="px-3 py-2 font-mono text-[var(--accent-green)]">{field.target}</td>
                      <td className="px-3 py-2 text-[var(--text-secondary)]">{field.description}</td>
                      <td className="px-3 py-2"><span className="badge-green text-[10px]">已映射</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <Wifi className={`h-4 w-4 ${healthy ? "text-[var(--accent-green)]" : "text-[var(--accent-amber)]"}`} />
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap gap-1">
          {(provider.fields ?? [provider.type ?? "provider"]).map((field) => (
            <span key={field} className="badge-blue text-[10px]">{field}</span>
          ))}
        </div>
        <button
          onClick={onTest}
...
function StatusCard({ label, value, tone }: { label: string; value: string; tone: "green" | "amber" }) {
  return (
    <div className="card flex items-center gap-3">
      <CheckCircle2 className={`h-4 w-4 ${tone === "green" ? "text-[var(--accent-green)]" : "text-[var(--accent-amber)]"}`} />
      <div className="min-w-0">
