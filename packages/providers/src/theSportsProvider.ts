// @lineupcast/providers — TheSportsProvider: placeholder for TheSports API
//
// All methods return null/empty with graceful fallback.
// Requires THESPORTS_API_KEY env var to function.
...
function getApiKey(): string | undefined {
  return process.env["THESPORTS_API_KEY"];
}
...
function buildMeta(apiKeyPresent: boolean): Provider {
  return {
    ...
    name: "TheSports API",
    description:
      "Comprehensive sports data API (football, basketball, etc.). " +
      "Requires API key from thesports.com.",
...
/**
 * Placeholder provider for TheSports API.
 *
 * Without a valid THESPORTS_API_KEY environment variable, all methods
 ...
 * Usage:
 * ```ts
 * // Set THESPORTS_API_KEY env var before importing
 * const provider = new TheSportsProvider();
 * ```
 */
export class TheSportsProvider implements DataProvider {
  readonly id = "thesports";
  ...
  async fetchUpcomingMatches(_league: string): Promise<Match[]> {
    try {
      ...
      // When implemented, call: GET /v1/football/match/list
      return [];
    } catch {
      return [];
    }
  ...
  async fetchMatch(matchId: string): Promise<Match> {
    try {
      // API integration not yet implemented.
      // When implemented, call: GET /v1/football/match/detail
  ...
  async fetchTeam(teamId: string): Promise<Team> {
    try {
      // API integration not yet implemented.
      // When implemented, call: GET /v1/football/team/detail
  ...
  async fetchSquad(_teamId: string): Promise<Player[]> {
    try {
      // API integration not yet implemented.
      // When implemented, call: GET /v1/football/team/players
      return [];
    } catch {
      return [];
    }
  ...
  async fetchLineup(matchId: string, teamId: string): Promise<Lineup> {
    try {
      // API integration not yet implemented.
      // When implemented, call: GET /v1/football/match/lineup
  ...
  async fetchMatchStats(matchId: string): Promise<MatchStats> {
    try {
      // API integration not yet implemented.
      // When implemented, call: GET /v1/football/match/statistics
  ...
  async fetchH2H(teamAId: string, teamBId: string): Promise<H2HRecord> {
    try {
      // API integration not yet implemented.
      // When implemented, call: GET /v1/football/team/vs
  ...
  async fetchForm(_teamId: string, _limit?: number): Promise<FormEntry[]> {
    try {
      // API integration not yet implemented.
      // When implemented, call: GET /v1/football/team/recent
      return [];
    } catch {
      return [];
    }
  ...
  async fetchPrediction(matchId: string): Promise<Prediction> {
    try {
      // TheSports does not provide prediction data.
